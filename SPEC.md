...
- **`wan2.7-image` 不支持 `negative_prompt` 参数**（阿里云官方文档：「对于不希望出现的
  ...
  > 这条是**普通版**的实测结论。2026-09-03 起两个图位跑不同模型：**主图** `wan2.7-image-pro`、
  > **附图** `wan2.7-image`（见 §13.18）。所以附图这一半仍在实测覆盖范围内，而 pro 是否也不收
  > `negative_prompt` **没有单独验证过** —— 但不验证没有故障风险：负向词并入正文在两种
  > 情况下都能工作，只是当 pro 其实支持独立参数时不够优。
...
池子原为 226 条；2026-09-02 验证 `--session-key` 改动时跑了一批 refill，aves +2 变成
227（紫辉林星蜂鸟、金尾蜂鸟）。重跑 `probe/simulate.py` 确认四项仍全绿，②③ **还是
189 天** —— 加条目不会改变这个下界，只会让「一轮」变长（227 天）。

跑了 730 / 1095 天、四个不同起始日（跨 ISO 周对齐），②③ **恒为 189 天**。这不是
巧合，是结构性下界：条目出窗口后最早能复现的时机就是下一个同类群日，
`ceil(183/7)*7 = 189`。所以余量看着只有 6 天，但它不会再缩。

### 9.2 最小安全水位 = 28 条/类群（`probe/simulate.py --scan` 实测）
...
断更是**单类群**事件。`probe/simulate.py --starve amphibia:26` 实测：
...
8. **`amphibia` 里混进了养殖经济鱼类，需要产品判断（未决）。** 实测入队的 32 条里有
   ...
   `probe/simulate.py --starve amphibia:26` 实测：全池 220 条、`QUEUE_LOW` 不响，730 天断更
...
10. **`RATIO` 目前发不出去（未实测）。** 页面期望主图 16:9 通栏、附图 4:3，而实际恒定
    ...
    > 2026-09-03 实测确认了「出方图」这一半：当天四张成品是 1280×1280（main）与
    > 1024×1024（sub），本地 `cwebp -resize W 0` 只按宽缩放、高按比例走，所以模型给的
    > 方图到最后还是方图。**没有验证的是另一半** —— 把 `size` 挪到 `parameters` 里到底
    > 收不收。换 `wan2.7-image-pro` 之后这个问题重新有了价值：pro 若认 `parameters.size`，
    > 花一张图的钱就能把这条长期缺陷修掉。目前仍未验证。
    > 当天下午的对比又添一条：两个模型的**原始返回**都是 2048×2048（四张 raw PNG 实测），
    > 所以 pro 的默认尺寸与普通版一致。注意这只说明「不传 size 时它给方图」，**不等于**
    > 它会拒绝 `parameters.size` —— 那一步仍然没试。
...
15. **调 `openclaw agent` 必须显式给 `--session-key`，否则 token 会翻几倍**（2026-09-02
    实测并已修）。不给 key 时所有调用都落进默认会话 `agent:<id>:main`，上下文只增不减。
    9/1 那次 refill 的实测账（数据取自 `~/.openclaw/agents/animal/sessions/<uuid>.jsonl`
    的 `usage` 字段）：43 个批次 / 93 次模型调用，`input` 37.8 万、`output` 5.2 万、
    `cacheRead` 430 万。其中 `cacheRead` 从 8,192 单调涨到 72,704，触顶后 openclaw 压缩
    上下文，一小时压了 6 次；压缩后重建上下文的那 5 次调用 `input` 分别是 53,698 /
    53,758 / 53,378 / 52,697 / 54,095，**合计 26.7 万，占 input 总量的 71%**。

    **这些历史对任务毫无价值**，所以那 71% 是纯浪费：每批只是「读一份固定 prompt + N 条
    清单 → 写 N 行 TSV」，跨批去重由 `refill-check.py` 拿 `queue.tsv` 做，从不依赖 agent
    记得上一批写了什么。改成每批一个 key 后单批实测（`REFILL_ONLY=aves BATCH=2 ROUNDS=1`）：
    2 次调用，`input` 11,218 / `output` 1,987 / `cacheRead` 9,216，合计 2.2 万，无压缩。

    口径要说清，别把改善说大：① 单批实测是 2 行，生产是 8 行/批，output 和清单会长一些，
    按 8 行推算约 2.6 万/批 × 43 ≈ 112 万，对 473 万是**降约 76%**，不是十倍；② `input`
    其实**略微上升**（每批都要重新加载引导，约 10K × 43 ≈ 43 万 > 原来的 37.8 万），省下
    的全在 `cacheRead`（430 万 → 约 40 万）；而 `cacheRead` 是折扣计价，所以折算成钱的
    省幅比 token 数的省幅小。真要再压，方向是**加大 `REFILL_BATCH`** 摊薄每批那 10K 起步
    成本（批数从 43 降到 22 就少一半），但一次写 16 行的质量未验证，不改。

    两个连带事项：**key 里必须带日期**（`refill-$TODAY-$slug-$r`）—— refill 每月 2 号跑，
    同名 key 会把上个月那个 session 整段历史加载回来，等于没改；**session 文件会变多**
    （每次 refill 约 40 个），清理用 `openclaw sessions cleanup`。`daily` 分支同样改了
    （`daily-$TODAY-$try`）：它一天只调一次，但默认会话的累积是**跨天**的，等于每天替前
    几天的历史付一次重建费；带上 `$try` 是因为 DUP 重试换的是题目，上一轮那半截被判重的
    正文留在上下文里只会干扰。

16. **`./run.sh verify` 通过不等于仓库干净。** 它的 git 那一项只查
    `git status --porcelain -- docs data/posts.jsonl data/content`（run.sh:131），也就是
    **发布产物**；`data/queue.tsv`、`run.sh` 自己被改了它一律看不见。这是有意的设计——
    verify 要回答的是"今天发出去的东西和仓库里的一致吗"，不是"我有没有忘记提交"。但两者
    容易混：本项目就有一次拿 `verify rc=0` 当过"没有未提交改动"的证据。要后者请直接看
    `git status --porcelain`（无路径限定）。

17. **换生图模型前先用 `GET {base}/compatible-mode/v1/models` 验证模型名，这一步零成本。**
    2026-09-03 用户对比后要求两个项目都从 `wan2.7-image` 换到 `wan2.7-image-pro`。**模型名
    在某个入口能用，不代表本项目打的那个端点收它** —— 而这里只有一个便宜的验证办法：模型
    列表接口是查询，不出图不计费，本次返回 12 个模型，`wan2.7-image-pro` 在其中。不做这一步
    就得等第二天早上出图失败才知道，而两个项目的图会同时出不来。

    列表通过之后**仍要真发一次请求**，因为列表只证明名字存在，不证明响应结构没变 ——
    `call_model` 是从 `output.choices[0].message.content[]` 里挑 `type=image` 的项取 URL 的，
    pro 若换了结构就会解析不出来。实测：`ok`，URL 正常，**耗时 39.3s**。

    **耗时是这次唯一的意外**：普通版 10 次样本稳定在 18–26s（那是含下载与转码的端到端间隔，
    纯 API 更短），pro 要 39.3s，约两倍。目前不构成问题 —— `IMG_TIMEOUT` 默认 120s 有三倍
    余量，两张图约 80s，而 animal 的 daily(11:25) → notify(12:00) 有 35 分钟。但**这个余量
    是三倍而不是十倍**，若哪天高峰期 pro 再慢一截就要看 `IMG_TIMEOUT`，所以把数字记在这里。

    改动面共五处，缺一处就会出现「以为换了其实没换」：两个 `.env` 的 `IMG_MODEL`（真正生效
    的），两侧 `gen-image.py` 里 `os.environ.get("IMG_MODEL", ...)` 的**回退默认值**，以及
    animal 编写侧那一份。回退值本可以留旧模型当降级路径，但那会让「现在到底在跑哪个」取决
    于 `.env` 有没有被读到 —— 两处写成同一个值就没有这个问题。

18. **模型按图位分开配：主图 `IMG_MODEL_MAIN`，附图 `IMG_MODEL_SUB`**（2026-09-03 当天第二次
    调整）。用户看了计费后要求「主图保留 pro、附图退回基础版」—— 附图在页面上只占一小块，
    为它付 pro 的钱不划算。两个项目同改，当前值 `main=wan2.7-image-pro`、`sub=wan2.7-image`。

    实现要点：`MODEL` 从单个字符串变成 `{"main": ..., "sub": ...}` 映射（与 `RATIO` 同形），
    `call_model(prompt, ratio)` 加第三个参数 `model`，唯一调用点在 `one()` 里而 `kind` 正好
    在手边。`kind` 未知时退到 `MODEL["sub"]`：那种情况本身是 bug，别让它顺手花贵的钱。

    **废弃旧变量必须吵出来，不能静默忽略。** 代码不再读 `IMG_MODEL`，若它还留在 `.env` 里，
    行为是「配置写了却完全不生效」—— 这是最难查的一类故障。所以两侧都加了一句：检测到
    `IMG_MODEL` 就往 stderr 打 WARN。这条与 §13.14 联动 —— cron 下 stderr 进 `logs/cron.log`，
    而那个文件正常应为空，于是它自动变成一道体检。告警分支已实测会触发（不是"应该会"）。

    顺带把 `model` 记进 `img_urls.jsonl`（两个项目都记，wiki 此前连 `prompt` 都不记）。
    在两个图位跑同一个模型的年代这个字段是可选的，一旦分开就成了必需：否则只能靠 `kind`
    反推当时的映射，而映射一改，历史记录就再也说不清哪张图是谁画的。**这次换模型的验证图
    就吃了这个亏** —— 它是直接调 `call_model` 出的，绕过 `log_url`，URL 只在 stdout 里，
    事后用户想复核时已经找不回来了。

    实测耗时补充：pro 两次样本 43.0s / 47.0s（比首测 39.3s 还慢些），基础版 18–26s，
    所以「pro 约两倍」站得住，`IMG_TIMEOUT` 120s 是 2.5 倍余量。

19. **`.gitignore` 写 `.env` 一条挡不住 `.env.bak.*`**（2026-09-03 在 wiki 侧补上）。
    gitignore 不做前缀匹配。animal 早就有 `.env.bak.*` 这条规则，wiki 一直漏着 —— 于是往
    `/opt/wiki` 里放一份 `.env` 备份就会变成未跟踪文件，而它和 `.env` 一样装着明文
    `IMG_API_KEY`，一次 `git add .` 就进公开仓库了。改 `.env` 前留备份是好习惯，但**备份放
    哪里取决于那个仓库的 gitignore 认不认**，别凭习惯放。放好后用
    `git check-ignore -v <file>` 确认，不要假定。
