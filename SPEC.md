...
1. **学名格式**：`^[A-Z][a-z]+ [a-z]+( [a-z]+)?$`（双名或三名法）
2. **GBIF rank**：必须 ∈ {`SPECIES`, `SUBSPECIES`}。`FAMILY`/`GENUS`/`ORDER` 一律拒
3. **GBIF taxonomicStatus**：必须 `ACCEPTED`（拒 `SYNONYM`——实测 `Puma concolor
   californica` 就是 SYNONYM，收进来会和正名重复）
4. **野生**：中文名不在家养名录（狗/猫/牛/羊/猪/鸡/鸭/马/家兔…），且学名不含
   `familiaris`/`domesticus`/`taurus` 等家养种加词
5. **中文名可得**：中文名必须在本地 zhwiki 索引中命中（`grep -c ":<名>$"`）——
...
`blacklist.txt` 初始收录（凡"一个中文词覆盖多个物种"的统称）：

```
老虎 狮子 豹 猎豹 熊 狼 狐狸 鹿 羚羊 猴 猿 鹰 隼 雕 鸮 鹤 天鹅 企鹅
鲸 海豚 鲨鱼 海豹 龟 鳄鱼 蛇 蜥蜴 蛙 蟾蜍 大象 犀牛 河马 长颈鹿 斑马
蝴蝶 甲虫 蚂蚁 蜜蜂 蜘蛛 章鱼 螃蟹 虾
```

`whitelist.txt` 初始收录（俗名即专名，虽有亚种但中文语境无歧义）：`美洲狮`。

`data/review-needed.tsv` 收**被闸门拒掉的行**（不是待审清单），供人工翻查有无误伤；
确认误伤就把该名补进 `whitelist.txt`，下次 refill 自动放行。
...
| 2 | `taxon-check.py` + 黑白名单 | 用 8 条用例验定：**必拒** 猫科/老虎/狮子/家犬；**必过** 孟加拉虎/美洲狮/薮猫/棕熊（后两条是 §0 抽样里亚种数最多的物种，专防日后有人重新引入数值判据）|
| 3 | agent 补中文名 → `queue.tsv` ≥220 条 | 闸门全过；`review-needed.tsv` 人工过一遍 |
