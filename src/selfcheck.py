...
GOOD = {
    ...
    "art": {
        ...
        # 附图只写画面里有什么。第一版照 prompt 的旧写法带了「浅色底，标本图谱式的平光」，
        # gen-image.py --dry-run 一跑发现那句在最终 prompt 里出现了两次 —— 底色与打光是
        # 脚本按类群附加的（FRAME["sub"]），prompt.md 已据此改掉那一行。
